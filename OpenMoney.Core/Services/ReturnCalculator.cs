...
{
    /// <summary>
    /// Modified Dietz return over [start, end].
    /// Externally-weighted to account for mid-period cash flows.
    /// </summary>
    ...
    /// <summary>
    /// Annualise a sub-year return using simple scaling; compound for multi-year.
    /// </summary>
        ...
        DateTime today)
    {
        ...
        foreach (var (name, start, end) in periods)
        {
            ...
            // Estimate start value by working backwards from current
            decimal startVal = EstimateValueAt(transactions, currentMarketValue, start, today);
            decimal endVal   = name == "alltime" ? currentMarketValue : currentMarketValue;
            // For simplicity end value is always current; start is estimated
            endVal = currentMarketValue;

            double years = (end - start).TotalDays / 365.25;
            var r = ModifiedDietz(startVal, endVal, flows, start, end);
    ...
    private static decimal CashFlow(Transaction t) =>
        t.Activity switch
        {
            ...
            ActivityType.ReinvestDividend => 0,   // internal reinvestment, no external flow
